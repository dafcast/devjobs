...
<div class="container mx-auto">
    <h1 class="text-center mt-20 text-2xl">Editar vacante: {{$vacante->titulo}}</h1>

    <form class="m-10 mx-auto w-full max-w-lg" action="{{route('vacantes.update', ['vacante' => $vacante->id])}}" method="POST">
        @csrf
        @method('put')
        ...
        <div class="mb-5 flex flex-wrap">
            ...
            <input 
                ...
                id="titulo"
                name="titulo"
                value="{{old('titulo') ? old('titulo') : $vacante->titulo}}"
            >
            @error('titulo')
        ...
        <div class="mb-5 flex flex-wrap">
            ...
            leading-tight focus:outline-none focus:bg-white text-gray-700 focus:border-gray-500 @error('experiencia') ring-2 ring-red-700 focus:ring-red-700 @enderror">
                ...
                @foreach($experiencias as $experiencia)
                    <option value="{{$experiencia->id}}" {{ ($vacante->experiencia->id == $experiencia->id) ? 'selected' :'' }}>{{$experiencia->experiencia}}</option>
                @endforeach
            </select>
        ...
        <div class="mb-5 flex flex-wrap">
            ...
            leading-tight focus:outline-none focus:bg-white text-gray-700 focus:border-gray-500 @error('ubicacion') ring-2 ring-red-700 focus:ring-red-700 @enderror">
                ...
                @foreach($ubicacions as $ubicacion)
                    <option value="{{$ubicacion->id}}" {{ ($vacante->ubicacion->id == $ubicacion->id) ? 'selected' :'' }}>{{$ubicacion->ubicacion}}</option>
                @endforeach
            </select>
        ...
        <div class="mb-5 flex flex-wrap">
            ...
            leading-tight focus:outline-none focus:bg-white text-gray-700 focus:border-gray-500 @error('salario') ring-2 ring-red-700 focus:ring-red-700 @enderror">
                ...
                @foreach($salarios as $salario)
                    <option value="{{$salario->id}}" {{ ($vacante->salario->id== $salario->id) ? 'selected' :'' }}>{{$salario->salario}}</option>
                @endforeach
            </select>
            @error('salario')
                <span class="bg-red-200 border-l-4 border-red-700 text-red-700 w-full p-2 text-sm mt-2" role="alert">
